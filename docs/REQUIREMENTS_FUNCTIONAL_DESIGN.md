...
表示条件: ログイン中のユーザーがシステム管理権限（`users`コレクションの`role`が`system_admin`）を持つ場合のみ、ヘッダーに「組織管理」タブが表示され、アクセス可能となる。
...
実行プロセス:
「作成」ボタンを押すと、クライアントアプリからFirebase Cloud FunctionsのOn-Call関数 `createOrganization` が直接呼び出される。

Cloud Functionの処理:
a. 権限チェック: 呼び出し元がシステム管理者であることを確認する。
b. アカウント発行: Firebase Authenticationで新しいユーザーアカウントを作成する。
c. データ作成: 
   - `organizations`コレクションに組織ドキュメントを作成。
   - `users`コレクションに管理者用ドキュメントを作成（`role: 'org_admin'`）。
   - デフォルトの大会データを作成。

B. 組織一覧の表示
このシステムに登録されている全ての組織を一覧で表示し、管理する機能。Firestoreの`organizations`コレクションを直接参照して表示する。
...
#### 機能
- **誰でも閲覧可能**: 認証不要。
- **トーナメント表表示**: 指定された大会(`tournaments`)の `matchGroups` データに基づき、トーナメント表を描画する。
...
ユーザーの権限（ロール）は、Firestoreの `users` コレクションの `role` フィールドで管理する。
ドキュメントIDはAuthenticationのUIDと一致させる（1対1の関係）。

#### システム管理者 (`role: 'system_admin'`):
- `users` コレクションの自身のドキュメントに `role: 'system_admin'` を持つ特権ユーザー。
- 「組織管理画面」にアクセスでき、新しい組織と組織管理者アカウントを発行できる。
- 全てのデータの閲覧権限を持つ（セキュリティルール上）。

#### 組織管理者 (`role: 'org_admin'`):
- 「組織管理画面」で発行された、各組織（団体）の管理者アカウント。
- `users` コレクションの `orgId` フィールドで自身の所属組織が紐付いている。
