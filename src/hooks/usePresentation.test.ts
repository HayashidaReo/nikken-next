...
describe("usePresentation", () => {
  ...
  beforeEach(() => {
    ...
    mockPresentationRequest.start
      .mockReset()
      .mockResolvedValue(mockPresentationConnection);
    mockPresentationRequest.getAvailability
      .mockReset()
      .mockResolvedValue(mockPresentationAvailability);
    mockPresentationAvailability.addEventListener.mockReset();

    // Presentation APIが利用可能な環境をシミュレート
    (globalThis as { PresentationRequest?: unknown }).PresentationRequest =
      jest.fn(() => mockPresentationRequest);
  ...
  describe("初期状態", () => {
    ...
    it("Presentation APIがサポートされていない場合の初期状態", () => {
      ...
      delete (globalThis as { PresentationRequest?: unknown })
        .PresentationRequest;
  ...
  describe("PresentationRequestの初期化", () => {
    it("正常に初期化される場合", async () => {
      ...
      expect(
        (globalThis as { PresentationRequest?: jest.Mock }).PresentationRequest
      ).toHaveBeenCalledWith([presentationUrl]);
    ...
    it("PresentationRequest初期化が失敗した場合はエラーが設定される", async () => {
      ...
      (globalThis as { PresentationRequest?: unknown }).PresentationRequest =
        jest.fn(() => mockPresentationRequestError);
  ...
  describe("プレゼンテーション開始", () => {
    beforeEach(() => {
      jest.clearAllMocks();
      mockPresentationAvailability.value = true;
      mockPresentationRequest.getAvailability.mockResolvedValue(
        mockPresentationAvailability
      );
      mockPresentationRequest.start.mockResolvedValue(
  ...
  describe("プレゼンテーション終了", () => {
    ...
    it("正常にプレゼンテーションを終了できる", async () => {
      ...
      const terminateHandler =
        mockPresentationConnection.addEventListener.mock.calls.find(
          call => call[0] === "terminate"
        )?.[1] as (() => void) | undefined;
