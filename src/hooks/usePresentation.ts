...
export function usePresentation(presentationUrl: string) {
    ...
    useEffect(() => {
        const isSupported = 'presentation' in navigator && 'PresentationRequest' in window;
    ...
    useEffect(() => {
        ...
        try {
            ...
            // 利用可能性をチェック
            presentation.getAvailability().then((availability) => {
    ...
    // メッセージ受信のリスナー設定
    const setMessageListener = useCallback((listener: (data: unknown) => void) => {
        ...
        return () => {
            if (state.connection) {
                state.connection.removeEventListener('message', handleMessage as EventListener);
            }
        };
    }, [state.connection]);
