...
export function usePresentation(presentationUrl: string) {
  ...
  const setupConnection = useCallback((connection: PresentationConnection, registerGlobal: boolean) => {
    // IDを保存
    localStorage.setItem(STORAGE_KEYS.PRESENTATION_CONNECTION_ID, connection.id);
    ...
    const handleClose = () => {
      ...
      localStorage.removeItem(STORAGE_KEYS.PRESENTATION_CONNECTION_ID);
      if (registerGlobal) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // 再接続を試みる
      const savedId = localStorage.getItem(STORAGE_KEYS.PRESENTATION_CONNECTION_ID);
      if (savedId) {
