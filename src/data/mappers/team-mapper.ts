...
export class TeamMapper {
  ...
  static toFirestoreForUpdate(
    ...
    if (team.representativeName !== undefined)
      updateData.representativeName = team.representativeName;
    if (team.representativePhone !== undefined)
      updateData.representativePhone = team.representativePhone;
    if (team.representativeEmail !== undefined)
      updateData.representativeEmail = team.representativeEmail;
    if (team.players !== undefined) {
      updateData.players = team.players.map(player =>
        this.playerToFirestore(player)
      );
    }
   ...
   */
  static toDomainsFromQuerySnapshot(
    docs: Array<{ id: string; data(): FirestoreTeamDoc }>
  ): Team[] {
    return docs.map(doc => {
      const data = doc.data();
  ...
  static validateAndConvert(
    doc: FirestoreTeamDoc & { id?: string }
