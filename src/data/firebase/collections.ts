...
export const clientCollections = {
    ...
    teams: (orgId: string, tournamentId: string) =>
        collection(db, "organizations", orgId, "tournaments", tournamentId, "teams"),

    /**
     * 特定の大会の matches サブコレクション
     */
    matches: (orgId: string, tournamentId: string) =>
        collection(db, "organizations", orgId, "tournaments", tournamentId, "matches"),
} as const;
...
export const clientDocs = {
    ...
    team: (orgId: string, tournamentId: string, teamId: string) =>
        doc(db, "organizations", orgId, "tournaments", tournamentId, "teams", teamId),

    /**
     * 特定の試合のドキュメント参照
     */
    match: (orgId: string, tournamentId: string, matchId: string) =>
        doc(db, "organizations", orgId, "tournaments", tournamentId, "matches", matchId),
} as const;
...
export const adminDocs = {
     ...
     */
    tournament: (orgId: string, tournamentId: string) =>
        adminDb.collection("organizations").doc(orgId).collection("tournaments").doc(tournamentId),
