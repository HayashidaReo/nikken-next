...
export class TeamData {
    ...
    async createTeam(team: Partial<Team>): Promise<string> {
        const teamsCollection = clientCollections.teams(this.orgId, this.tournamentId);
    ...
    async getAllTeams(): Promise<Team[]> {
        const teamsCollection = clientCollections.teams(this.orgId, this.tournamentId);
        const querySnapshot = await getDocs(query(teamsCollection, orderBy("createdAt", "desc")));
     ...
     * チームの承認状態を更新
     */
    async updateApprovalStatus(teamId: string, isApproved: boolean): Promise<void> {
        const teamDoc = clientDocs.team(this.orgId, this.tournamentId, teamId);
     ...
     * バッチ処理でチーム情報を一括更新
     */
    async batchUpdateTeams(updates: Array<{ teamId: string; data: Partial<Team> }>): Promise<void> {
