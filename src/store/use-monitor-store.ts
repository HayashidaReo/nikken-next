...
interface MonitorState {
    ...
    // アクション
    initializeMatch: (match: Match, tournamentName: string, courtName: string) => void;
...
export const useMonitorStore = create<MonitorState>((set, get) => ({
    ...
    // アクション
    initializeMatch: (match: Match, tournamentName: string, courtName: string) => {
        set({
    ...
    setPlayerHansoku: (player: "A" | "B", hansoku: number) => {
        const currentState = get();
        const currentPlayer = player === "A" ? currentState.playerA : currentState.playerB;
        const opponent = player === "A" ? currentState.playerB : currentState.playerA;
        ...
        // 相手の得点を更新（最大2点まで）
        const newOpponentScore = Math.min(2, Math.max(0, opponent.score + opponentScoreChange));
