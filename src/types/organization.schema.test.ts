...
describe("Organization Schema Validation", () => {
    describe("organizationSchema", () => {
        ...
        it("representativeEmailが空文字列の場合はエラー", () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                expect(result.error.issues[0].message).toBe("団体代表者メールアドレスは必須です");
            }
        });
    ...
    describe("organizationCreateSchema", () => {
        ...
        it("有効な組織作成データを受け入れる", () => {
            const result = organizationCreateSchema.safeParse(validOrganizationCreate);
            expect(result.success).toBe(true);
        });
    ...
    describe("organizationCreateWithAccountSchema", () => {
        ...
        it("有効な組織作成（アカウント付き）データを受け入れる", () => {
            const result = organizationCreateWithAccountSchema.safeParse(validOrganizationCreateWithAccount);
            expect(result.success).toBe(true);
        });
        ...
        it("adminEmailが空文字列の場合はエラー", () => {
            ...
            expect(result.success).toBe(false);
            if (!result.success) {
                expect(result.error.issues[0].message).toBe("管理者メールアドレスは必須です");
            }
        });

        it("adminEmailが無効な形式の場合はエラー", () => {
            const invalid = { ...validOrganizationCreateWithAccount, adminEmail: "invalid-admin-email" };
            const result = organizationCreateWithAccountSchema.safeParse(invalid);
            expect(result.success).toBe(false);
            if (!result.success) {
                expect(result.error.issues[0].message).toBe("正しいメールアドレスを入力してください");
        ...
        it("adminPasswordが6文字未満の場合はエラー", () => {
            const invalid = { ...validOrganizationCreateWithAccount, adminPassword: "123" };
            const result = organizationCreateWithAccountSchema.safeParse(invalid);
            expect(result.success).toBe(false);
            if (!result.success) {
                expect(result.error.issues[0].message).toBe("パスワードは6文字以上である必要があります");
            }
        });

        it("adminPasswordがちょうど6文字の場合は成功", () => {
            const valid = { ...validOrganizationCreateWithAccount, adminPassword: "123456" };
        ...
        it("orgId、createdAt、updatedAtは含まれない", () => {
            ...
            const result = organizationCreateWithAccountSchema.safeParse(withExtraFields);
            expect(result.success).toBe(true);
            if (result.success) {
                expect("orgId" in result.data).toBe(false);
                expect("createdAt" in result.data).toBe(false);
