...
export function TimeAdjuster({
  ...
  const adjustTime = useCallback(
    (type: TimeAdjustType) => {
      ...
      const { minutes, seconds } = splitSecondsToMinutesAndSeconds(
  ...
  const minutesUpPress = useLongPress(() => adjustTime("minutes-up"), {
    delay: longPressDelay,
    interval: longPressInterval,
    disabled,
  });

  const minutesDownPress = useLongPress(() => adjustTime("minutes-down"), {
    delay: longPressDelay,
    interval: longPressInterval,
    disabled,
  });

  const secondsUpPress = useLongPress(() => adjustTime("seconds-up"), {
    delay: longPressDelay,
    interval: longPressInterval,
    disabled,
  });

  const secondsDownPress = useLongPress(() => adjustTime("seconds-down"), {
    delay: longPressDelay,
    interval: longPressInterval,
    disabled,
  });
  ...
  return (
    <div
      className={`flex items-center justify-center ${styles.height} ${className}`}
    >
      <div
        className={`font-mono font-bold flex items-center justify-center ${styles.time} ${styles.spacing}`}
      >
        {/* 分の表示と調整 */}
