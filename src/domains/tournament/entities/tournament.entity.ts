...
export class TournamentEntity {
  ...
  get isValid(): boolean {
    return (
      ...
      this.data.tournamentDate instanceof Date &&
      !isNaN(this.data.tournamentDate.getTime()) &&
      this.data.location.trim().length > 0 &&
      this.data.defaultMatchTime > 0 &&
      this.data.courts.length > 0 &&
      this.data.courts.every(
        court =>
          court.courtId.trim().length > 0 && court.courtName.trim().length > 0
      )
    );
  }
