...
import {
  splitPlayerName,
  validatePlayerNames,
} from "@/lib/utils/player-name-utils";
...
export class TeamFormConverter {
  ...
  static toTeamCreate(formData: TeamFormData): TeamCreate {
    ...
    // displayName を生成
    const playersWithDisplayNames =
      DisplayNameService.generateDisplayNames(players);
  ...
  static validateFormData(formData: TeamFormData): {
    ...
    const invalidPlayerIndices = validatePlayerNames(
      formData.players.map(p => p.fullName)
    );
    invalidPlayerIndices.forEach(index => {
      ...
      if (!fullName) {
        ...
        messages.push(
          `選手${index + 1}: 選手名は「姓 名」の形式で入力してください`
        );
      }
    });
