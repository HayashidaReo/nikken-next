...
jest.mock('@/domains/team/services/display-name.service', () => ({
    DisplayNameService: {
        generateDisplayNames: jest.fn((players: Array<{ lastName: string; firstName: string }>) =>
            players.map((p) => ({ ...p, displayName: `${p.lastName} ${p.firstName[0]}` }))
        ),
    },
}));
...
describe('TeamFormConverter', () => {
    ...
    describe('validateFormData', () => {
        ...
        test('無効なメールアドレスでエラーになる', () => {
            const invalidData = { ...validFormData, representativeEmail: 'invalid-email' };
            const result = TeamFormConverter.validateFormData(invalidData);
            expect(result.isValid).toBe(false);
            expect(result.errors.some(err => err.includes('メールアドレス'))).toBe(true);
        });
