...
│   │   ├── form-field.test.tsx # テストファイル
│   │   └── ...
│   ├── organisms/        # 4. Organisms (有機体)
│   │   ├── login-form.tsx  # (例: ログインフォーム機能全体)
│   │   ├── login-form.test.tsx # テストファイル
│   │   ├── team-edit-form.tsx
...
├── hooks/                # 共有カスタムフック
│   ├── use-presentation.ts
│   ├── use-presentation.test.ts # テストファイル
│   └── ...
│
├── lib/                  # ライブラリ / ユーティリティ
...
│   ├── use-matches.ts
│   ├── use-matches.test.ts # テストファイル
│   ├── use-teams.ts
│   └── use-teams.test.ts   # テストファイル
│
├── store/                # Zustandのストア定義
│   ├── use-auth-store.ts
│   ├── use-auth-store.test.ts # テストファイル
│   └── ...
│
├── types/                # Zodスキーマと型定義
│   ├── team.schema.ts
│   ├── team.schema.test.ts    # テストファイル
│   ├── match.schema.ts
│   └── match.schema.test.ts   # テストファイル
│
...
**基本ルール**: テストファイルは、テスト対象ファイルと同じディレクトリに配置する

#### ファイル命名規則
- **テストファイル名**: `{対象ファイル名}.test.{拡張子}`
- **例**:
  - `Button.tsx` → `Button.test.tsx`
  - `utils.ts` → `utils.test.ts`
  - `team.schema.ts` → `team.schema.test.ts`
...
├── components/atoms/
│   ├── Button.tsx
│   └── Button.test.tsx        # ← 同じフォルダに配置
├── lib/
│   ├── utils.ts
│   └── utils.test.ts          # ← 同じフォルダに配置
├── types/
│   ├── team.schema.ts
│   └── team.schema.test.ts    # ← 同じフォルダに配置
```
...
#### ① コンポーネントテスト (`.test.tsx`)
- **対象**: React コンポーネント
- **使用ライブラリ**: `@testing-library/react`
- **テスト内容**: レンダリング、イベントハンドリング、プロパティ渡し、アクセシビリティ

#### ② ロジックテスト (`.test.ts`)
- **対象**: ユーティリティ関数、ビジネスロジック
- **使用ライブラリ**: Jest
- **テスト内容**: 入力・出力の検証、エラーケース、エッジケース

#### ③ スキーマテスト (`.schema.test.ts`)
- **対象**: Zodスキーマ
- **使用ライブラリ**: Jest + Zod
- **テスト内容**: バリデーション成功・失敗、エラーメッセージ検証

#### ④ フックテスト (`.test.ts`)
- **対象**: カスタムReactフック、TanStack Query、Zustand
- **使用ライブラリ**: `@testing-library/react-hooks`
- **テスト内容**: 状態変更、副作用、リアルタイム連携

### 11. テスト除外ルール

**テスト対象外**:
- **Pages** (`app/` ディレクトリ): E2Eテストで対応
- **Templates**: 主にレイアウトのため、組み込みテストで対応
- **型定義のみ**: 実行時ロジックがない純粋な型定義ファイル

## 🔥 Firebase & Backendルール

### 12. Firebase SDKの使い分け
...
### 13. Cloud Functions (Backend Logic)
